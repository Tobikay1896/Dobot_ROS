...
\begin{itemize}
  ...
  \item \textbf{ROS2-Integration:}
        ...
        können dieses Topic abonnieren und die aktuelle
        Kinematik in Echtzeit empfangen.
  \item \textbf{Pick-\&-Place (Teaching):}
  ...
  \item \textbf{Würfel-Tracking:}
        Ein interaktiv in der Szene verschiebbarer Ziel-Würfel
        dient als Bewegungssollwert für den Roboterarm.
        Das Tracking funktioniert auch ohne verbundenen COM-Port.
\end{itemize}

Die folgenden Abschnitte beschreiben zunächst die Systemumgebung
(Abschnitt~\ref{sec:system}), dann Einrichtung und Installation
(Abschnitt~\ref{sec:einrichtung}), das Robotermodell
(Abschnitt~\ref{sec:robotermodell}), die Gesamtarchitektur
(Abschnitt~\ref{sec:architektur}), das kinematische Modell
(Abschnitt~\ref{sec:kinematik}) und schließlich alle
Implementierungsdetails (Abschnitt~\ref{sec:impl}).
...
Dieser Abschnitt dokumentiert die Hardware- und Softwareumgebung,
in der die Extension entwickelt und betrieben wird.
...
\texttt{setup\_ros\_env.bat} gesetzt.

\subsection{Virtuelle Entwicklungsumgebung und Python-Pakete}

Die Extension benötigt \textbf{keine} separate lokale Python-Umgebung.
Alle Laufzeitabhängigkeiten laufen ausschließlich innerhalb der
eingebetteten Python-Umgebung von Isaac Sim (\texttt{python.bat}).
...
\label{sec:einrichtung}
% ===================================================================

Dieser Abschnitt beschreibt den Aufbau des Projekts, erklärt wie
Isaac Sim die Extension lädt, und zeigt die nötigen Schritte zur
Erstinstallation.
...
\begin{tabular}{lp{9.5cm}}
  ...
  Datei & Beschreibung \\
  \midrule
  \texttt{extension.py}       & Hauptlogik: ROS-Node, Kinematik,
                                DC-Kopplung, UI, Pick-\&-Place, Würfel-Tracking \\
...
Der Eintrag \texttt{[[python.module]]} teilt Isaac Sim mit, welches
Python-Modul geladen werden soll und wo es liegt.
Beim Aktivieren der Extension importiert Isaac Sim das Modul
\texttt{dobot\_ros} aus dem Verzeichnis \texttt{.} (dem
Extension-Stammordner selbst). Python lädt dabei zuerst
\texttt{\_\_init\_\_.py}, die nur eine einzige Zeile enthält:

\begin{lstlisting}
from .extension import DobotBridgeExtension
\end{lstlisting}

Damit steht \texttt{DobotBridgeExtension} im Namespace des Moduls
zur Verfügung. Isaac Sim durchsucht den Namespace automatisch nach
einer Klasse, die \texttt{omni.ext.IExt} implementiert, und findet
\texttt{DobotBridgeExtension}. Anschließend ruft das Kit-Framework
\textbf{zwei Lifecycle-Methoden} auf:

\begin{description}[leftmargin=5cm, style=nextline]
  \item[\texttt{on\_startup(ext\_id)}]
    Wird aufgerufen, sobald die Extension aktiviert wird
    (Häkchen im Extension-Manager oder Autostart).
    Hier werden Zustandsvariablen initialisiert, das UI-Fenster
    gebaut und der Frame-Update-Callback registriert.
  \item[\texttt{on\_shutdown()}]
    Wird aufgerufen, wenn die Extension deaktiviert oder Isaac Sim
    beendet wird. Trennt die Dobot-Verbindung, stoppt alle
    Hintergrundthreads und gibt UI-Ressourcen frei.
\end{description}

Solange die Extension nicht korrekt als \texttt{omni.ext.IExt}
erkannt wird --- etwa weil \texttt{extension.toml} den falschen
Modulnamen oder Pfad enthält --- werden weder \texttt{on\_startup}
noch \texttt{on\_shutdown} jemals aufgerufen, und die Extension
erscheint im Manager als nicht ladbar.

\subsection{Installations-Hilfsskript (\texttt{install\_extension.py})}

\paragraph{Wozu dient das Skript?}

Isaac Sim kennt zwei Wege, eine Extension zu finden:

\begin{enumerate}
  \item \textbf{Suchpfad im Extension-Manager} (Entwicklung):
        Im Extension-Manager unter
        \textit{Fenster $\to$ Extensions $\to$ Zahnrad $\to$ Paths}
        kann der Quellordner direkt eingetragen werden.

  \item \textbf{Roaming-Extensions-Verzeichnis} (Installation):
        Isaac Sim durchsucht beim Start automatisch
        \texttt{\%APPDATA\%\textbackslash{}NVIDIA Corporation\textbackslash{}Omniverse\textbackslash{}Extensions}.
        Wer die Extension dauerhaft ohne manuellen Pfadeintrag
        betreiben oder auf einem Produktionsrechner einsetzen möchte,
        kopiert den Ordner dorthin. Genau das erledigt
        \texttt{install\_extension.py}.
\end{enumerate}

\paragraph{Wann wird das Skript benötigt?}

\begin{itemize}
  \item Deployment auf einem Rechner ohne Entwicklungsumgebung
  \item Saubere Installation ohne Git-Artefakte und Cache-Dateien
  \item Schnelle Ersteinrichtung auf einem neuen System
\end{itemize}

\paragraph{Verwendung}

\begin{lstlisting}[style=batch, caption={install\_extension.py -- Verwendung}]
# Standard-Installation in das Omniverse-Roaming-Verzeichnis:
python install_extension.py

# Eigenes Zielverzeichnis und Ordnername:
...
\begin{enumerate}
  \item \textbf{Extension-Pfad eintragen:}
        Im Extension-Manager unter
        \textit{Fenster $\to$ Extensions $\to$ Zahnrad $\to$ Paths}
        den Ordner \texttt{dobot\_ros} als Suchpfad hinzufügen.
        Alternativ \texttt{install\_extension.py} ausführen, um
        die Extension in das Roaming-Verzeichnis zu kopieren.

  \item \textbf{Extension aktivieren:}
        Im Extension-Manager nach \texttt{dobot\_ros} suchen
        und aktivieren.
...
% ===================================================================
\section{Robotermodell}
\label{sec:robotermodell}
% ===================================================================

Bevor die Architektur und Implementierung der Extension beschrieben
werden, ist ein Verständnis des Robotermodells notwendig: Ohne ein
korrekt importiertes Modell kann die Extension nicht mit der
Simulationsgelenkstruktur kommunizieren.

\subsection{URDF-Import}

URDF (\textit{Unified Robot Description Format}) ist ein XML-basiertes
Format zur vollständigen Beschreibung eines Roboters: Körper (Links),
Gelenke (Joints), 3D-Meshes, Trägheitsmomente und Kollisionsgeometrien.
Isaac Sim kann URDF-Dateien direkt importieren und daraus ein
simulierbares USD-Modell erzeugen.

Ohne ein korrekt importiertes Modell gibt es in Isaac Sim keine
Gelenkstruktur, auf die die Dynamic-Control-API zugreifen kann.
Die Extension setzt voraus, dass das Modell als
\textbf{Artikulation} geladen ist -- d.\,h. \texttt{base\_link}
trägt die \texttt{ArticulationRootAPI} und alle Gelenke sind als
DOFs (Degrees of Freedom) registriert. Nur dann liefert
\texttt{get\_articulation()} einen gültigen Handle.

\begin{description}[leftmargin=4cm, style=nextline]
  \item[Artikulation]
    In Isaac Sim bezeichnet \emph{Artikulation} eine zusammenhängende
    Kette von starren Körpern, die über Gelenke verbunden sind und
    gemeinsam von der Physik-Engine simuliert werden. Der Prim, der
    \texttt{ArticulationRootAPI} trägt, ist der Wurzelknoten dieser
    Kette -- alle Kindknoten mit Gelenken gehören automatisch dazu.
    Ohne dieses Schema behandelt Isaac Sim die einzelnen Meshes als
    unabhängige Starrkörper ohne kinematische Verbindung.

  \item[DOF (Degree of Freedom)]
    Jedes bewegliche Gelenk in der Artikulation bekommt beim Start
    der Physik-Simulation eine interne Indexnummer -- seinen
    \emph{DOF-Index}. Über diesen Index spricht die
    Dynamic-Control-API das Gelenk an: Winkel lesen,
    Sollwert setzen. \glqq Als DOF registriert\grqq{} bedeutet,
    dass das Gelenk von der Physik-Engine erkannt wurde und
    einen gültigen Index hat.

  \item[Handle]
    Ein \emph{Handle} ist ein numerischer Bezeichner, den
    \texttt{get\_articulation()} zurückgibt, sobald die Physik
    läuft und die Artikulation gefunden wurde. Er ist vergleichbar
    mit einem Datei-Handle beim Öffnen einer Datei: Erst wenn er
    gültig ist (\(\neq\) \texttt{INVALID\_HANDLE}), können alle
    weiteren DC-API-Aufrufe auf den Roboter zugreifen. Deshalb
    initialisiert die Extension die DC-Verbindung lazy -- erst
    nach dem Drücken von \emph{Play}.
\end{description}

\subsection{Gelenkstruktur des Dobot Magician}

Der Dobot Magician hat ein Parallelogrammgestänge im Inneren, das
kinematisch dafür sorgt, dass \texttt{link\_6} (Endeffektor-Flansch)
stets horizontal bleibt. Im URDF wird dieses Gestänge \emph{nicht}
als eigene Links modelliert -- stattdessen bildet die Kette direkt:

\begin{center}
\texttt{base\_link}
$\xrightarrow{\text{joint\_1 (Z, }\pm180^{\circ})}$
\texttt{link\_1}
$\xrightarrow{\text{joint\_2 (Z, 0--90}^{\circ}\text{)}}$
\texttt{link\_2}
$\xrightarrow{\text{joint\_5 (-Z, 0--120}^{\circ}\text{)}}$
\texttt{link\_5}

\texttt{link\_5}
$\xrightarrow{\text{joint\_6 (-Z, }\pm180^{\circ})}$
\texttt{link\_6}
$\xrightarrow{\text{joint\_suction (fixed, +4\,cm)}}$
\texttt{link\_suction}
\end{center}

\texttt{joint\_3} und \texttt{joint\_4} existieren im URDF nicht --
das Parallelogramm ist kinematisch in die Winkelformeln von
\texttt{joint\_5} und \texttt{joint\_6} eingerechnet
(siehe Abschnitt~\ref{sec:kinematik}).
...
Mit dem Robotermodell als Grundlage lässt sich nun die
Gesamtarchitektur der Extension beschreiben. Diese gliedert sich in
vier klar voneinander getrennte Schichten, die von oben (UI) bis
unten (Hardware) aufeinander aufbauen.
...
Die gesamte Logik verteilt sich auf zwei Klassen, die klar
voneinander getrennte Verantwortlichkeiten haben:
...
\begin{description}[leftmargin=4.5cm, style=nextline]
  \item[\texttt{DobotROSNode}]
    ...
    Vakuumgreifer und RC-Servo-Ansteuerung.
  ...
  \item[\texttt{DobotBridgeExtension}]
    ...
    Hintergrundthreads für Pick-\&-Place-Sequenzwiedergabe
    und Würfel-Tracking.
\end{description}

\subsection{Datenfluss}

\subsubsection{Richtung 1: Echter Roboter \texorpdfstring{$\to$}{->} Simulation (Digital Twin)}
...
\begin{enumerate}
  ...
  \item Kinematische Transformation liefert
...
\subsubsection{Richtung 2: Simulation / UI \texorpdfstring{$\to$}{->} Echter Roboter}
...
Bevor die einzelnen Implementierungskomponenten betrachtet werden,
ist das kinematische Modell zu verstehen, da es den Kern der
Datenverarbeitung im Digital-Twin-Betrieb bildet.
...
\begin{notebox}
Durch das Parallelogrammgestänge des Dobot Magician gilt
näherungsweise $j_3 \approx 0{{}^\circ}$ (Unterarm bleibt in
Weltkoordinaten horizontal), sofern das Gestänge korrekt eingestellt ist.
\end{notebox}
...
\begin{equation}
  \theta_1 = j_1 + \Delta_\text{Yaw} = j_1 - 12{{}^\circ}
  \label{eq:j1}
\end{equation}

Der Wert ist als Modul-Konstante \texttt{J1\_YAW\_OFFSET\_DEG = -12.0}
...
if pose is not None and len(pose) >= 8:
    ...
    j1 = j1 + J1_YAW_OFFSET_DEG   # -12 Grad (kalibriert)
...
Dieser Abschnitt beschreibt die technischen Details der Implementierung:
wie Module geladen werden, wie die beiden Hauptklassen intern aufgebaut
sind, wie die Dynamic-Control-Kopplung funktioniert und wie das
Threading-Modell aufgebaut ist.
...
\texttt{DobotROSNode} ist die Hardware-Schicht der Extension.
Sie kapselt die serielle Verbindung zum Dobot und den ROS2-Node.

\subsubsection{Initialisierung (\texttt{\_\_init\_\_})}
...
\subsubsection{Methoden-Referenz}
...
% -------------------------------------------------------------------
\subsection{Dynamic-Control-Kopplung}
\label{sec:dc}
% -------------------------------------------------------------------

Die Dynamic-Control-API verbindet die kinematischen Werte aus
\texttt{DobotROSNode} mit der Gelenkstruktur in der Simulation.

\paragraph{Initialisierungsproblem:}
...
\paragraph{Lazy-Initialisierung:}
...
\paragraph{Gelenkwinkel setzen -- \texttt{\_drive\_joints()}:}
...
Die Kinematik-Formeln halten alle Ausgaben innerhalb dieses Bereichs.
\end{warnbox}
...
\texttt{DobotBridgeExtension} ist die oberste Schicht der Extension.
Sie verbindet UI, Simulation und die Hardware-Klasse
\texttt{DobotROSNode} und verwaltet alle Hintergrundthreads.

\subsubsection{Lebenszyklusmethoden}
...
\begin{description}[leftmargin=4.5cm, style=nextline]
  ...
  \item[\texttt{on\_shutdown()}]
    Dock-Task canceln (verhindert \glqq extension object is still
    alive\grqq-Warnung), \texttt{\_cleanup()} aufrufen,
    Fenster zerstören.
\end{description}

\subsubsection{Frame-Update-Ablauf (\texttt{\_on\_update()})}

Dieser Callback wird pro Frame vom Isaac-Sim-Event-System aufgerufen
und ist der zentrale Takt der Digital-Twin-Kopplung:
...
\subsubsection{Ressourcenfreigabe (\texttt{\_cleanup()})}
...
% -------------------------------------------------------------------
\subsection{Threading-Modell}
\label{sec:threading}
% -------------------------------------------------------------------

Die Extension verwendet drei Threads, um blockierende Operationen
vom Isaac-Sim-Hauptthread zu entkoppeln:
...
Isaac-Sim-Hauptthread einfrieren. Im Hintergrundthread blockiert
sie nur diesen Thread; Isaac Sim läuft ungehindert weiter.
...
(\texttt{\_set\_status()}) sind durch den GIL von CPython atomisch.
...
Das UI-Fenster wird beim Start der Extension aufgebaut und
automatisch neben dem Property-Panel gedockt. Es besteht aus
zwei Tabs und einem dauerhaft sichtbaren Status-Dashboard.
...
\begin{tabular}{lp{8cm}}
  ...
  Würfel spawnen / entfernen & Ziel-Würfel ein-/ausblenden;
                                funktioniert auch ohne COM-Port \\
...
Dieser Abschnitt beschreibt die beiden Hauptfunktionen der Extension,
die über die einfache Fernsteuerung hinausgehen: das Pick-\&-Place-Szenario
und das Würfel-Tracking. Abschließend folgt eine Schritt-für-Schritt-Anleitung
für den täglichen Betrieb.

% -------------------------------------------------------------------
\subsection{Pick-\&-Place-Szenario (Teaching)}
\label{sec:teachin}
% -------------------------------------------------------------------

\subsubsection{Datenstruktur der Wegpunkte}
...
\begin{description}[leftmargin=4.5cm, style=nextline]
  ...
  \item[\textit{Saugen EIN/AUS}]
    ...
    Ermöglicht präzises Einteachen von Greif- und Ablegepositionen.

  \item[\textit{Löschen}]
    Setzt Wegpunktliste, Saugzustand und Zähler zurück.
\end{description}

\subsubsection{Wiedergabe}

Die Wiedergabe läuft in einem dedizierten Hintergrundthread
(\texttt{dobot\_play}), damit die blockierende
\texttt{wait\_for\_cmd()}-Schleife den Isaac-Sim-Hauptthread
nicht einfriert.
...
Nach dem letzten Wegpunkt oder nach Abbruch fährt der Roboter
automatisch zur Nullposition zurück und der Sauger wird deaktiviert.
\texttt{\_on\_stop\_sequence\_clicked()} setzt das Stop-Flag und
leert die interne Dobot-Bewegungs-Queue sofort.

% -------------------------------------------------------------------
\subsection{Ziel-Würfel-Tracking}
\label{sec:tracking}
% -------------------------------------------------------------------

Der Ziel-Würfel ist ein orangenes 2,5\,cm-Objekt in der Isaac-Sim-Szene,
das frei verschoben werden kann. Die Tracking-Funktion liest seine
Position aus der USD-Stage und sendet entsprechende Fahrbefehle an
den Dobot. Das Tracking funktioniert auch ohne verbundenen COM-Port --
in diesem Fall wird die Zielposition nur angezeigt.

\subsubsection{Würfel erstellen (\texttt{\_spawn\_target\_cube()})}

\begin{enumerate}
  \item Alten Würfel entfernen (Idempotenz).
  \item Startposition aus \texttt{DobotROSNode.last\_pose}
        (mm $\to$ m: Division durch 1000); Fallback auf
        Home-Position wenn nicht verbunden.
...
\subsubsection{Tracking-Schleife}

Statt des Würfelzentrums wird der Normalenvektor von
Würfelzentrum zu aktueller TCP-Position berechnet und der
TCP auf die dem Roboter zugewandte Würfelfläche ausgerichtet:

\begin{lstlisting}[caption={Tracking-Thread-Kernlogik}]
CUBE_HALF_MM = 12.5   # halbe Kantenlaenge in mm
...
\paragraph{Normalbetrieb (Digital Twin):}
...
\paragraph{Pick-\&-Place-Workflow (Teaching):}
...
Die ROS2-Anbindung ermöglicht externen Knoten, die Gelenkzustände
des Dobot in Echtzeit zu empfangen.
...
Alle magischen Zahlen und Konfigurationswerte sind in
\texttt{constants.py} zentralisiert, damit sie an einer Stelle
...
\begin{tabular}{lrl}
  ...
  \texttt{SERVO\_MIN\_DUTY} & $0{,}05$ & 1\,ms-Puls $= 0^{\circ}$ \\
  \texttt{SERVO\_MAX\_DUTY} & $0{,}10$ & 2\,ms-Puls $= 180^{\circ}$ \\
  \bottomrule
\end{tabular}
\end{center}
